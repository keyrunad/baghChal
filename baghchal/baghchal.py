...
class Board:
    def draw(self, screen):
        ...
        for row in range(1,5):
            for column in range(1,5):
                ...
                pygame.draw.rect(screen, BLACK,[((120)*column-30), ((120)*row-30), 120, 120],4)
                
    def draw_tiger(self, screen):
        #draw tiger
        tiger = pygame.image.load('images/tiger.png')
    ...
    def draw_moves(self, screen):
        #draw blue squares on all positions with possible move for characters
...
def main():
    ...
    selected = 0 #flag to know a piece is selected, 1 when piece is selected
    ...
    help = False #flag for help screen, True when help screen is on display, false otherwise
    done = False
    reset = True #flag to reset the game variables when game is restarted
	
    def displayvars():
        #display variables on the screen
        font2 = pygame.font.Font('fonts/ErasBoldITC.ttf', 25)
        if turn == 1 and start == True and game_end == False:
            text1 = "Goat's Turn."
        ...
        screen.blit(textSurf,(640,15))
        font3 = pygame.font.Font('fonts/ErasBoldITC.ttf',35)
        
        textSurf = font3.render(text6, True, BLACK)
    ...
    while not done:
        ...
        if refresh == True:
            ...
            elif help == False: #draw game screen when help is false
        ...
        for event in pygame.event.get(): #get user inputs
            if event.type == pygame.MOUSEBUTTONDOWN and start == False and help == False: #start the game when user clicks start button when start is False, help is False
                ...
                if column >= 640 and column <=790 and row >=60 and row <=100:
                    ...
                    board.play_menu()
                    continue
            if event.type == pygame.MOUSEBUTTONDOWN and help == False:
                pos = pygame.mouse.get_pos()
                column = pos[0]
                row = pos[1]
                if column >= 810 and column <=910 and row >=500 and row <=540:
                    ...
                    help = True
                    board.play_menu()
            if event.type == pygame.MOUSEBUTTONDOWN and help == True:
                pos = pygame.mouse.get_pos()
                column = pos[0]
                row = pos[1]
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and game_end == True and start == True and help == False: #reset the game after game is over and if restart button is clicked
                    pos = pygame.mouse.get_pos()
                    column = pos[0]
                    row = pos[1]
                    if column >= 640 and column <=790 and row >=60 and row <=100:
                        ...
                        board.play_menu()
                        msg = 'Game restarted.'
            if event.type == pygame.MOUSEBUTTONDOWN and game_end == True and start == True and help == False: #quit if quit button is clicked
                    pos = pygame.mouse.get_pos()
                    column = pos[0]
                    row = pos[1]
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and game_end == False and start == False and help == False: #quit if quit button is clicked
                    pos = pygame.mouse.get_pos()
                    column = pos[0]
                    row = pos[1]
                    if column >= 810 and column <=910 and row >=60 and row <=100:
            ...
            elif game_end == False and start == True and help == False: #when game is started but not over
